...
export const surgeriesService = {
  ...
  async getByPatient(patientId: string): Promise<Surgery[]> {
    try {
      const response = await apiClient.get<Surgery[]>(API_ENDPOINTS.SURGERIES.BY_PATIENT(patientId));
      return response.data || [];
    } catch (error) {
  ...
  async getByStatus(status: string): Promise<Surgery[]> {
    try {
      const response = await apiClient.get<Surgery[]>(API_ENDPOINTS.SURGERIES.BY_STATUS(status));
      return response.data || [];
    } catch (error) {
      console.error('Error fetching surgeries by status:', error);
      throw error;
    }
  },
  ...
  async getPendingConsent(): Promise<Surgery[]> {
    try {
      const response = await apiClient.get<Surgery[]>(API_ENDPOINTS.SURGERIES.PENDING_CONSENT);
      return response.data || [];
    } catch (error) {
      console.error('Error fetching pending consent surgeries:', error);
      throw error;
    }
  },
  ...
  async update(id: string, data: Partial<Surgery>): Promise<Surgery> {
    const response = await apiClient.put<Surgery>(API_ENDPOINTS.SURGERIES.BY_ID(id), data);
    return response.data;
  },

  async updateStatus(id: string, status: string): Promise<Surgery> {
    const response = await apiClient.put<Surgery>(`${API_ENDPOINTS.SURGERIES.UPDATE_STATUS(id)}?status=${status}`);
    return response.data;
  },
